...
class AuthControllerTest extends RestDocsTest {
    ...
    void authorize() throws Exception {
        ...
        final ResultActions perform = mockMvc.perform(
                post("/auth/refresh")
                        .content(
                                toRequestBody(new TokenRequest("access.token.value", "refresh.token.value")))
    ...
    void saveMember() throws Exception {
        ...
        final ResultActions perform = mockMvc.perform(
                post("/auth/save")
                        .content(
                                toRequestBody(new MemberSaveRequest("정해진", "http://some-url-to-profile-image", "01000000000", Role.ROLE_USER, new OAuthRequest("client-id", AuthType.APPLE))))
                        .contentType(MediaType.APPLICATION_JSON));
    ...
    void afterSocialLogin() throws Exception {
        ...
        final ResultActions perform = mockMvc.perform(
                post("/auth/login")
                        .content(
                                toRequestBody(new OAuthRequest("client-id", AuthType.KAKAO)))
                        .contentType(MediaType.APPLICATION_JSON));
