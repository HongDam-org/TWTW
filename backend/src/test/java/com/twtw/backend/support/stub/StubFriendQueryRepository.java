...
public class StubFriendQueryRepository implements FriendQueryRepository {
    ...
    public Optional<Friend> findByTwoMemberId(final UUID loginMemberId, final UUID memberId) {
        return map.values().stream()
                .filter(friend ->
                        (friend.getToMember().getId().equals(loginMemberId) && friend.getFromMember().getId().equals(memberId))
                                || (friend.getFromMember().getId().equals(loginMemberId) && friend.getToMember().getId().equals(memberId)))
                .findFirst();
    }
    ...
    public List<Friend> findByMember(final Member member) {
        return map.values().stream()
                .filter(friend -> friend.getToMember().equals(member) || friend.getFromMember().equals(member))
                .toList();
    }

    @Override
    public List<Friend> findByMemberAndFriendStatus(final Member member, final FriendStatus friendStatus) {
        return map.values().stream()
                .filter(friend -> (friend.getToMember().equals(member) || friend.getFromMember().equals(member)) && friend.getFriendStatus().equals(friendStatus))
                .toList();
    }
    ...
    public List<Friend> findByMemberAndMemberNicknameContaining(final UUID memberId, final String nickname) {
        return map.values().stream()
                .filter(friend -> friend.getFriendStatus() == FriendStatus.ACCEPTED && ((friend.getToMember().getId().equals(memberId) && friend.getFromMember().getNickname().contains(nickname))
