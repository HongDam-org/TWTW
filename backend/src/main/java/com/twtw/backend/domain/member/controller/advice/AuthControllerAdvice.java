...
public class AuthControllerAdvice {
    ...
    public ResponseEntity<ErrorResponse> refreshTokenInfoMismatch(final InvalidFriendMemberException e) {
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(new ErrorResponse(e.getMessage()));
    }

    @ExceptionHandler(RefreshTokenValidationException.class)
    public ResponseEntity<ErrorResponse> refreshTokenValidation(final InvalidFriendMemberException e) {
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(new ErrorResponse(e.getMessage()));
    }
}
