...
public class FriendQueryRepositoryImpl implements FriendQueryRepository {
    ...
    public Optional<Friend> findByTwoMemberId(final UUID loginMemberId, final UUID memberId) {
        return Optional.ofNullable(jpaQueryFactory
                .selectFrom(friend)
                .where((friend.toMember.id.eq(loginMemberId).and(friend.fromMember.id.eq(memberId)))
                        .or(friend.fromMember.id.eq(loginMemberId).and(friend.toMember.id.eq(memberId))))
                .fetchFirst());
    }
    ...
    public List<Friend> findByMemberAndMemberNickname(final Member member, final String nickname) {
        return jpaQueryFactory
                .selectFrom(friend)
                .where((friend.toMember.eq(member).and(friend.fromMember.nickname.eq(nickname)))
                        .or(friend.fromMember.eq(member).and(friend.toMember.nickname.eq(nickname))))
                .fetch();
    }
}
